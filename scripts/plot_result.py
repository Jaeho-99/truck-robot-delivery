...
WGS84_TO_KATEC = Transformer.from_crs("EPSG:4326", KATEC_CRS, always_xy=True)
KATEC_TO_WGS84 = Transformer.from_crs(KATEC_CRS, "EPSG:4326", always_xy=True)
...
def _load_nodes(processed_path: Path) -> list[dict[str, Any]]:
    if not processed_path.is_file():
        raise FileNotFoundError(
            f"processed instance not found: {processed_path}"
        )
...
def _load_boundaries(
    ...
    for feature in features:
        ...
        for ring in _iter_exterior_rings(feature["geometry"]):
            ...
            east, north = WGS84_TO_KATEC.transform(lonlat[:, 0], lonlat[:, 1])
            local_xy = (
...
def _internal_labels(nodes: list[dict[str, Any]]) -> list[str]:
    ...
    customers = [node["label"] for node in nodes if node["type"] == "customer"]
    parkings = [node["label"] for node in nodes if node["type"] == "parking"]
    if len(depot) != 1:
...
def _heuristic_arcs(
    ...
    def route_order(item: tuple[str, Any]) -> int:
        try:
            ...
            raise ValueError(
                f"invalid truck id in routes: {item[0]!r}"
            ) from exc
    ...
    for truck_key, route in sorted(routes.items(), key=route_order):
        ...
        for stop in route:
            if not isinstance(stop, dict):
                raise TypeError(
                    f"route stop for truck {truck} must be an object"
                )
            kind = stop.get("kind")
            if kind == "cust":
...
def _legend(ax: Any) -> Any:
    handles = [
        Line2D(
            ...
            marker="*",
            color="none",
            markerfacecolor=DEPOT_FC,
            markeredgecolor=DEPOT_EC,
            markersize=18,
            linestyle="None",
            label="Main depot",
        ),
        Line2D(
            ...
            marker="s",
            color="none",
            markerfacecolor=PARK_FC,
            markeredgecolor=PARK_EC,
            markersize=12,
            linestyle="None",
            label="Parking node",
        ),
...
def _fleet_box(
    ...
    return ax.legend(
        dummy,
        [f"Trucks used: {n_trucks}", f"Robots used: {n_robots}"],
        loc="upper left",
        bbox_to_anchor=(1.0, box.y0 - 0.03),
        fontsize=12,
        framealpha=1.0,
        borderpad=1.0,
        labelspacing=0.7,
...
_DECORATOR_JS = (
    "https://unpkg.com/leaflet-polylinedecorator@1.6.0"
...
// Arrowheads get their own pane above the marker pane (600) and below the
// tooltip pane (650): nothing can cover a direction marker, and the pane
// stays click-through so an arrow never swallows a node's tooltip.
map.createPane('arrows');
map.getPane('arrows').style.zIndex = 640;
map.getPane('arrows').style.pointerEvents = 'none';

const gT = L.featureGroup(), gR = L.featureGroup(), gN = L.featureGroup();

// Arrow tips stop `gap` pixels short of the destination node instead of
// landing on its centre. Two consequences: the node marker cannot hide
// the tip, and arrows converging on one node fan out around it rather
// than stacking on a single point. A segment too short to hold its arrow
// gets none -- a waiting-style stop pair sits on one physical parking, so
// its arc has zero length and would otherwise project to NaN.
const ARROWS = [];
function arrowPath(a, b, gap) {
  const pa = map.latLngToLayerPoint(a), pb = map.latLngToLayerPoint(b);
  const len = pa.distanceTo(pb);
  if (!(len > gap + 3)) return [];
  const t = (len - gap) / len;
  return [[a, map.layerPointToLatLng(
    L.point(pa.x + (pb.x - pa.x) * t, pa.y + (pb.y - pa.y) * t))]];
}
function arrow(a, b, color, size, gap, group) {     // route direction marker
  // 100% puts the arrow *tip* at the end of the (shortened) segment: the
  // symbol draws its barbs backwards from the offset point, so each edge
  // shows where it ends, not only which way it runs. The white outline
  // keeps overlapping arrowheads apart from each other and from the map.
  const deco = L.polylineDecorator([], {patterns: [{offset: '100%',
    repeat: 0, symbol: L.Symbol.arrowHead({pixelSize: size, polygon: true,
      pathOptions: {pane: 'arrows', color: '#ffffff', weight: 1.2,
        opacity: 0.95, fillColor: color, fillOpacity: 1,
        interactive: false}})}]});
  ARROWS.push({deco: deco, a: a, b: b, gap: gap});
  deco.addTo(group);
}
function redrawArrows() {          // pixel gaps must follow the zoom level
  ARROWS.forEach(x => x.deco.setPaths(arrowPath(x.a, x.b, x.gap)));
}
D.truck_seg.forEach(s => {
  const col = TCOL[(s.k - 1) % TCOL.length];
  L.polyline([s.a, s.b], {color: col, weight: 4, opacity: 0.9})
    .bindTooltip('Truck ' + s.k).addTo(gT);
  arrow(s.a, s.b, col, 13, 13, gT);
});
D.robot_seg.forEach(s => {
  const col = RCOL[((s.k - 1) * 2 + (s.r - 1)) % RCOL.length];
  L.polyline([s.a, s.b], {color: col, weight: 2.5,
    opacity: 0.9, dashArray: '5,4'})
    .bindTooltip('Truck ' + s.k + ' - Robot ' + s.r).addTo(gR);
  arrow(s.a, s.b, col, 10, 8, gR);
});
...
map.on('zoomend', redrawArrows);   // the view must exist before projecting
redrawArrows();
...
def _boundary_latlng(boundary_path: Path) -> list[list[list[float]]]:
    ...
    for feature in geojson.get("features") or []:
        for ring in _iter_exterior_rings(feature["geometry"]):
            rings.append(
                [[round(lat, 6), round(lon, 6)] for lon, lat, *_ in ring]
            )
    return rings
...
def _write_html(
    ...
    *,
    ...
    at = {
        node["label"]: [round(float(a), 6), round(float(o), 6)]
    ...
    def segments(
        ...
        out = []
        for arc in arcs:
            record = {"a": at[arc["i"]], "b": at[arc["j"]], "k": int(arc["k"])}
            if with_robot:
                record["r"] = int(arc["r"])
            out.append(record)
        return out
    ...
    data = {
        ...
        "customers": [
            {
                **marker(node, node["label"]),
                "served_by": (
                    "robot" if node["label"] in robot_customers else "truck"
                ),
            }
            for node in nodes
        ...
        "parkings": [
            marker(node, node["label"].rsplit("#", 1)[0])
        ...
        "n_robots": len(
            {(int(arc["k"]), int(arc["r"])) for arc in robot_arcs}
        ),
    }
    html = (
        _HTML_TEMPLATE.replace("__DECORATOR_JS__", _DECORATOR_JS)
...
def plot_result(
    ...
    *,
    ...
    processed_path = (
        (processed_path or inferred_processed).expanduser().resolve()
    )
    ...
    for arc in [*truck_arcs, *robot_arcs]:
        if (
            arc.get("i") not in node_by_label
            or arc.get("j") not in node_by_label
        ):
    ...
    robot_customers = {
        ...
        for label in (arc["i"], arc["j"])
        if node_by_label[label]["type"] == "customer"
    }
    customers = [node for node in nodes if node["type"] == "customer"]
    truck_customers = [
        node for node in customers if node["label"] not in robot_customers
    ]
    robot_customer_nodes = [
        node for node in customers if node["label"] in robot_customers
    ]
    ...
    if all_boundary_xy:
        ...
        margin = 0.3
        ax.set_xlim(
            float(combined[:, 0].min()) - margin,
            float(combined[:, 0].max()) + margin,
        )
        ax.set_ylim(
            float(combined[:, 1].min()) - margin,
    ...
    html_path = _write_html(
        ...
        robot_arcs=robot_arcs,
        robot_customers=robot_customers,
    )
