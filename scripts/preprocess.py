...
RAW_KEYS = {
    "instance_id", "region", "seed", "n_customers",
    "num_parking_copies", "n_zones", "zones", "origin_katec", "nodes",
    "alpha_traffic", "alpha_ped",
}
...
def _read_json(path: Path) -> Any:
    with path.open("r", encoding="utf-8") as handle:
        return json.load(
            handle, object_pairs_hook=_unique_json_object,
...
def _exact_keys(value: Any, expected: set[Any], path: str) -> Mapping[Any, Any]:
...
def _validate_payload(path: Path, payload: Any, expected_size: int) -> dict[str, Any]:
    ...
    n_customers = _integer(raw["n_customers"], f"{path}: n_customers",
                           positive=True)
    ...
    copies = _integer(raw["num_parking_copies"],
                      f"{path}: num_parking_copies", positive=True)
    ...
    if zone_ids != expected_zones or {str(zone) for zone in zone_ids} != set(zones_raw):
        raise ValueError(f"{path}: zones must be contiguous string keys 0..{n_zones - 1}")
    zones: dict[int, dict[str, str]] = {}
    for zone in range(n_zones):
        info = _exact_keys(zones_raw[str(zone)], ZONE_KEYS,
                           f"{path}: zones[{zone}]")
        if (not isinstance(info["admi_cd"], str)
                or not isinstance(info["admi_nm"], str)
                or not info["admi_cd"] or not info["admi_nm"]):
            raise TypeError(f"{path}: invalid metadata for zone {zone}")
        zones[zone] = {
            "admi_cd": info["admi_cd"], "admi_nm": info["admi_nm"]}
    ...
    for index, value in enumerate(nodes_raw):
        ...
        if kind not in NODE_TYPES:
            raise ValueError(f"{path}: nodes[{index}].type is invalid: {kind!r}")
        ...
        node_xy.append((_real(node["x"], f"{path}: nodes[{index}].x"),
    ...
    customer_idx = np.flatnonzero(node_type_array == "customer").astype(np.int64)
    ...
    if customer_idx.size != n_customers:
        raise ValueError(f"{path}: customer node count disagrees with n_customers")
...
def _load_cell_table(path: Path) -> _CellTable:
    ...
    if not path.is_file():
        raise FileNotFoundError(f"KATEC cell coordinate file not found: {path}")
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for line_number, row in enumerate(reader, start=2):
            if None in row or any(row[column] is None or row[column] == ""
                                  for column in expected_columns):
                raise ValueError(f"{path}:{line_number}: incomplete cell row")
            cell_id = row["cell_id"]
            if cell_id in cell_ids:
                raise ValueError(f"{path}:{line_number}: duplicate cell_id {cell_id}")
            ...
            try:
                ...
                raise ValueError(
                    f"{path}:{line_number}: KATEC coordinates must be integers") from exc
...
def _ring_membership(points: np.ndarray,
                     ring: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def _polygon_covers(points: np.ndarray, rings: tuple[np.ndarray, ...]) -> np.ndarray:
...
class _CellZoneLocator:
    ...
    def locate_many(self, points: np.ndarray) -> np.ndarray:
        ...
        return np.min(np.where(tied, candidates, sentinel), axis=1).astype(np.int64)


class _PolygonZoneLocator:
    method = POLYGON_ZONE_METHOD

    def __init__(self, parts: tuple[_PolygonPart, ...], n_zones: int,
...
def _polygon_locator(polygon_groups: Mapping[str, list[Any]],
                     instance: Mapping[str, Any],
                     fallback: _CellZoneLocator) -> _PolygonZoneLocator:
    name_to_zone = {
    ...
    for name, polygons in polygon_groups.items():
        ...
        for polygon_index, polygon in enumerate(polygons):
            ...
            for ring_index, ring_value in enumerate(polygon):
                ...
                if (ring.ndim != 2 or ring.shape[1] < 2 or ring.shape[0] < 4
            ...
            parts.append(_PolygonPart(
                zone=zone,
                rings=tuple(transformed_rings),
                bbox=(float(all_points[:, 0].min()),
...
def _locator_key(instance: Mapping[str, Any], use_polygons: bool) -> tuple[Any, ...]:
    zone_schema = tuple(
...
def _get_locator(instance: Mapping[str, Any], cell_table: _CellTable,
                 polygon_groups: Mapping[str, list[Any]] | None,
                 cache: dict[tuple[Any, ...], _ZoneLocator]) -> _ZoneLocator:
...
class _LegDecomposer:
    def __init__(self, locator: _ZoneLocator, n_zones: int,
                 n_samples: int, scaling_factor: float):
    ...
    def decompose(self, start: np.ndarray, end: np.ndarray) -> np.ndarray:
        ...
        if zones.shape != (self.n_samples,) or (
                (zones < 0) | (zones >= self.n_zones)).any():
...
def _path_decomposition(start: np.ndarray, end: np.ndarray,
                        convention: str, legs: _LegDecomposer) -> np.ndarray:
    corner_x = np.asarray([end[0], start[1]], dtype=np.float64)
    corner_y = np.asarray([start[0], end[1]], dtype=np.float64)
    x_first = (legs.decompose(start, corner_x)
               + legs.decompose(corner_x, end))
    if convention == "x_first":
        return x_first
    y_first = (legs.decompose(start, corner_y)
               + legs.decompose(corner_y, end))
...
def _build_geometry(instance: Mapping[str, Any], params: Params,
                    locator: _ZoneLocator) -> tuple[np.ndarray, ...]:
    xy = instance["node_xy"]
    n_nodes = xy.shape[0]
    ...
    legs = _LegDecomposer(
        locator, n_zones, params.geometry.n_samples, scaling)
    ...
    def calculate(i: int, j: int) -> tuple[float, float, float, float]:
        ...
        distance = float((abs(xy[i, 0] - xy[j, 0])
                          + abs(xy[i, 1] - xy[j, 1])) * scaling)
        truck_time = float(
            np.dot(zone_km, instance["alpha_traffic"])
            / params.truck.speed_kmh * 60.0)
        robot_time = float(
            np.dot(zone_km, instance["alpha_ped"])
            / params.robot.speed_kmh * 60.0)
    ...
    _validate_geometry(
        d, tau_truck, tau_robot, decomposition_sum,
        instance["alpha_traffic"], instance["alpha_ped"], params)
    return d, tau_truck, tau_robot


def _validate_geometry(d: np.ndarray, tau_truck: np.ndarray,
                       tau_robot: np.ndarray, decomposition_sum: np.ndarray,
                       alpha_traffic: np.ndarray, alpha_ped: np.ndarray,
                       params: Params) -> None:
    for name, matrix in (("d", d), ("tau_truck", tau_truck),
                         ("tau_robot", tau_robot)):
...
def _metadata(instance: Mapping[str, Any], params: Params,
              locator: _ZoneLocator, git_commit: str, zone_source_hash: str,
              node_zone_mismatch_count: int) -> dict[str, Any]:
...
def _existing_hash(path: Path) -> str | None:
    ...
    try:
        ...
        if meta_array.shape != () or meta_array.dtype.kind != "U":
            return None
        meta = json.loads(
            str(meta_array.item()), object_pairs_hook=_unique_json_object,
            parse_constant=_reject_json_constant)
...
def _process_one(raw_path: Path, output_path: Path, size: int,
                 params: Params, polygon_groups: Mapping[str, list[Any]] | None,
                 cell_table: _CellTable,
                 locator_cache: dict[tuple[Any, ...], _ZoneLocator],
                 git_commit: str, zone_source_hash: str,
                 force: bool) -> tuple[str, int, int]:
    ...
    node_zone_mismatch_count = int(np.count_nonzero(
        assigned_node_zone != instance["node_zone"]))
    d, tau_truck, tau_robot = _build_geometry(instance, params, locator)
    meta = _metadata(
        instance, params, locator, git_commit, zone_source_hash,
    ...
    arrays = {
        ...
        "meta": np.asarray(json.dumps(
            ...
            allow_nan=False), dtype=np.str_),
    }
    _atomic_write(output_path, arrays)
    return "written", node_zone_mismatch_count, int(instance["node_xy"].shape[0])
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--size", choices=("5", "10", "20", "50", "100", "all"),
                        default="all")
...
def main() -> None:
    ...
    if DONG_POLYGON_PATH.is_file():
        ...
        if zone_method != POLYGON_ZONE_METHOD:
            raise RuntimeError("zone-source method disagrees with polygon availability")
        zone_note = (
    ...
    for split in selected_splits:
        for size in selected_sizes:
            ...
            if not source_dir.is_dir():
                raise FileNotFoundError(f"raw instance directory not found: {source_dir}")
            raw_paths = sorted(source_dir.glob("*.json"))
            if not raw_paths:
                raise FileNotFoundError(f"no JSON instances under {source_dir}")
            ...
            for raw_path in raw_paths:
                ...
                status, mismatches, node_count = _process_one(
                    raw_path, output_path, size, params, polygon_groups,
                    cell_table, locator_cache, git_commit, zone_source_hash,
                    args.force)
                if status == "skipped":
                    skipped += 1
                else:
