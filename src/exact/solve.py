...
from common.params import load_problem
...
def run_model(inst, config, model_name,
              time_limit_sec=300, mip_gap=0.0, symmetry_breaking=True,
              fix_binaries=None,
    ...
    d_ij = {(i, j): float(inst["d"][i, j])
            for i in N for j in N if i != j}
    tau_truck = {(i, j): float(inst["tau_truck"][i, j])
                 for i in N0 for j in Nplus if i != j}
    tau_robot = {(i, j): float(inst["tau_robot"][i, j])
                 for i in N0_hat for j in Nplus_hat if i != j}

    # ---- parameters ----
    s_kc = {(k, c): config.truck.service_time_min for k in K for c in C}
    s_hat = {(k, r, c): config.robot.service_time_min
             for k in K for r in R_k[k] for c in C}
    zeta_load = {(k, r, p): config.robot.load_time_min
                 for k in K for r in R_k[k] for p in P}
    zeta_unload = {(k, r, p): config.robot.unload_time_min
                   for k in K for r in R_k[k] for p in P}
    beta_k = {k: config.truck.capacity for k in K}
    beta_hat = {(k, r): config.robot.capacity
                for k in K for r in R_k[k]}
    phi_hat_param = {(k, r): config.robot.range_km
                     for k in K for r in R_k[k]}
    ...
    gammahat_fixed = {(k, r): config.robot.fixed_cost
                      for k in K for r in R_k[k]}
    gamma_fuel = {k: config.truck.fuel_cost_per_min for k in K}
    gamma_env = {k: config.truck.env_cost_per_min for k in K}
    gammahat_fuel = {(k, r): config.robot.fuel_cost_per_min
                     for k in K for r in R_k[k]}
    gammahat_env = {(k, r): config.robot.env_cost_per_min
                    for k in K for r in R_k[k]}
    ...
    if m.SolCount == 0:
        res = {"name": model_name, "status": STATUS_LOOKUP.get(m.Status, f"UNKNOWN_{m.Status}"), "obj": None,
    ...
    res = {
        "name": model_name, "status": STATUS_LOOKUP.get(m.Status, f"UNKNOWN_{m.Status}"),
...
def _to_exact_instance(data: ProcessedInstance, config: SharedParams,
                       size: int) -> dict:
    """Reindex one-depot NPZ arrays to the MILP's 0/C/P/D convention."""
    ...
    def reindex(matrix):
        result = np.ascontiguousarray(matrix[np.ix_(source_index, source_index)])
    ...
    return {
        ...
        "K": K,
        "R_k": {k: list(range(
            1, config.fleet.n_robots_per_truck + 1)) for k in K},
        "lam": {c: int(value) for c, value in
                zip(C, data.demand, strict=True)},
        "e_c": {c: float(value) for c, value in
                 zip(C, data.e, strict=True)},
        "l_c": {c: float(value) for c, value in
                 zip(C, data.l, strict=True)},
        "labels": labels,
        "park_groups": [groups[key] for key in sorted(
...
def _parser():
    ...
    parser.add_argument("--size", type=int, choices=(5, 10),
                        required=True)
...
def main():
    ...
    if args.limit < 0 or args.time_limit <= 0 or args.mip_gap < 0:
        raise ValueError("--limit/--mip-gap must be nonnegative and "
    ...
    if not source.is_dir():
        raise FileNotFoundError(f"processed instance directory not found: {source}")
    ...
    for path in paths:
        ...
        result = run_model(
            inst, config, model_name=instance_id,
            time_limit_sec=args.time_limit, mip_gap=args.mip_gap,
