...
def main():
    ...
    if args.iterations <= 0 or args.seeds <= 0:
        raise ValueError("--iterations and --seeds must be positive")
    ...
    for instance_id, params in cases:
        for seed in range(args.seeds):
            ...
            solution, obj, stats = solve_alns(params, iters=args.iterations,
                                              seed=seed)
            elapsed = time.perf_counter() - started
            route_path = out_dir / f"{instance_id}_s{seed}.json"
            with route_path.open("w", encoding="utf-8") as f:
                json.dump({"instance_id": instance_id, "seed": seed,
                           "obj": obj, "stats": stats,
                           "routes": solution.routes}, f, indent=2)
            rows.append({"instance_id": instance_id, "seed": seed,
                         "obj": obj, "runtime_s": round(elapsed, 3),
    ...
    if rows:
        ...
        with (out_dir / "test_metadata.json").open(
