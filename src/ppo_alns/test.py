"""Evaluate graph-free PPO-ALNS checkpoints on precomputed test instances.

Each instance/seed pair has its own action generator. Serial evaluation and
Windows spawn workers execute the same evaluator and return results in input
order. Checkpoint loading, worker cleanup, and result publication live here so
testing needs no separate common runtime or policy-evaluation module.

Example from the repository root::

    python src/ppo_alns/test.py --size 20 --workers 2 --run-label example

``--run-label`` changes output directories only. Select a different model with
``--checkpoint``; the default path remains under ``models/``.
"""
...
from ppo_alns.ppo import REWARD_MODES
...
# Runtime diagnostics are invoked once by the parent process.
...
# Spawn-safe evaluation workers and ordered result collection.
...
def run_evaluation_cli(package, args, *, main_started=None):
    """Load a checkpoint, evaluate jobs, and publish complete result files."""
    main_started = (
...
def _parser():
    """Define evaluation options without loading the selected checkpoint."""
...
def main():
    """Evaluate the requested checkpoint and publish ordered test results."""
    started = time.perf_counter()
    run_evaluation_cli(
        "ppo_alns", _parser().parse_args(), main_started=started
    )
