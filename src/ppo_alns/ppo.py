"""Graph-free PPO-ALNS policy, environment, transport, and training.

The nine global search features feed the actor and critic. Each selected action
applies one destroy/repair transition in ``alns.py``; rollout collection and PPO
updates are implemented below in the same module. Process workers transport
CPU observations and keep their own environment state and random generator.

Example from the repository root::

    python src/ppo_alns/train.py --size 20 --device cpu

``train.py`` owns command-line options and result files; ``test.py`` loads the
same checkpoint format for evaluation. This module does not run on import.
"""
...
from common.artifacts import atomic_open
from common.parallel_env import ParallelVecALNS as ParallelVecALNS
...
class FeatureObservation:
    ...
    def to(self, device):
        return type(self)(num_nodes=self.num_nodes, g=self.g.to(device))

    def to_dict(self):
        return {"num_nodes": self.num_nodes, "g": self.g}


class FeatureBatch(FeatureObservation):
    @classmethod
    def from_data_list(cls, observations):
        if not observations:
            raise ValueError("cannot batch an empty observation list")
        return cls(g=torch.cat([item.g for item in observations], dim=0))
...
def global_features(
    ...
    eps = max(
        float(pr.alpha_traffic.max()) - 1.0,
        float(pr.alpha_ped.max()) - 1.0,
        1e-9,
    )
    rho_cong = min(max((congestion_ratio(pr, served) - 1.0) / eps, 0.0), 1.5)
    ...
    is_current_best = (
        1.0 if it > 0 and abs(current_cost - best_cost) <= 1e-9 else 0.0
    )
    return torch.tensor(
        [
            [
...
@dataclass
class PPOConfig:
    """Model/search defaults stored in every checkpoint for reproducibility.

    CLI overrides are supplied by ``train.py``. A rollout contains
    ``t_rollout * n_envs`` transitions; ``n_updates`` rounds the requested
    training budget down to complete rollouts.
    """
...
class ActorCritic(nn.Module):
    """Independent actor and critic MLPs over the nine global features."""
...
class RolloutBuffer:
    """Store rollout-time observations, actions, values, and log probabilities.

    Observations are flattened in time-major order: for two environments,
    indices 0 and 1 belong to step 0, and indices 2 and 3 belong to step 1.
    Minibatches use that same order before applying a shared permutation.
    """
    ...
    def add(self, obs, actions, log_probs, values, rewards, dones):
        """Store one vector step: obs is a list of n_envs FeatureObservation objects
    ...
    def minibatches(self, n_minibatch, generator=None):
        ...
        Yields (graph FeatureBatch, actions, log_probs, advantages, returns,
        ...
        for chunk in perm.chunk(n_minibatch):
            ...
            batch = FeatureBatch.from_data_list([self.obs[i] for i in idx])
...
class ALNSEnv:
    ...
    FeatureObservation object with g_t attached as .g. done=True only at
    ...
    def _obs(self):
        ...
        data = (
            ...
            else FeatureObservation(num_nodes=0)
        )
...
class VecALNS:
    ...
    def _record_timings(self, started, workers, step_seconds, reset_seconds):
        primary = (
            "step"
            if any("step" in phases for phases in workers.values())
            else "reset"
        )
        totals = {
            i: phases[primary]["total_seconds"]
            for i, phases in workers.items()
        ...
        self.last_timings = {
            ...
            "worker_mean_seconds": (
                sum(totals.values()) / len(totals) if totals else 0.0
            ),
        }
    ...
    def reset(self):
        ...
        self._record_timings(
            started, timings, 0.0, time.perf_counter() - started
        )
    ...
    def step(self, actions):
        ...
        if len(actions) != len(self.envs):
            raise ValueError(
                f"expected {len(self.envs)} actions, got {len(actions)}"
            )
        checked = []
        for env, action in zip(self.envs, actions):
            if isinstance(action, bool):
                raise ValueError(
                    "ALNS actions must be integer indices, not bool"
                )
...
def validate_observation(data):
    ...
    if not isinstance(data, FeatureObservation):
        raise TypeError("PPO observation must be a FeatureObservation object")
    if set(data.to_dict()) != {"num_nodes", "g"}:
        raise ValueError(
            "PPO observation must contain exactly num_nodes and g"
        )
...
def decode_observation(payload, codec="direct"):
    ...
    data = FeatureObservation(num_nodes=0)
...
def make_envs(
    ...
    *,
    ...
    """Build matching serial or process environments with identical seeds.

    Instance sampling remains in the parent for both backends. A process
    worker owns one environment, its local instance cache, and its RNG.
    """
...
def save_checkpoint(model, cfg, norms, metadata, path):
    """Atomically publish the policy weights and their experiment settings."""
...
def train(
    ...
    *,
    ...
    try:
        stats.update(
            {
                ...
                "worker_pids": list(vec.worker_pids),
                "replay_benchmark": False,
            }
        )
        print(
            "[reset] constructing initial solutions for all environments",
            flush=True,
        )
        ...
        print(
            f"[reset] complete in {stats['initial_reset_seconds']:.2f}s",
            flush=True,
        )
        ...
        for upd in range(cfg.n_updates):
            ...
            for vector_step in range(cfg.t_rollout):
                ...
                with torch.no_grad():
                    ...
                    batch = FeatureBatch.from_data_list(obs)
                ...
                for worker_id, phases in timings.get("workers", {}).items():
                    for timing in phases.values():
                        ...
                        worker_graph_seconds += timing.get(
                            "graph_seconds", 0.0
                        )
                        worker_cache_sizes[worker_id] = timing.get(
                            "cache_size", 0
                        )
                ...
                if (vector_step + 1) % 32 == 0:
                    ...
                    rate = ((vector_step + 1) * cfg.n_envs) / max(
                        elapsed, 1e-9
                    )
                    print(
                        f"[rollout upd={upd} t={vector_step + 1}/{cfg.t_rollout}] "
            ...
            with torch.no_grad():
                last_v = model.get_value(
                    FeatureBatch.from_data_list(obs).to(device)
                ).cpu()
            ...
            metrics.update(
                {
                    ...
                    "update_wall_seconds": time.perf_counter()
                    - update_started,
...
def evaluate_instance(model, cfg, builder, pr, seed=0, sample=True):
    ...
    for _ in range(cfg.search_iterations):
        with torch.no_grad():
            batch = FeatureBatch.from_data_list([obs]).to(device)
