...
class RolloutBuffer:
    ...
    def add(self, obs, actions, log_probs, values, rewards, dones):
        """Store one vector step: obs is a list of n_envs Data objects
...
class ALNSEnv:
    ...
    Data object with g_t attached as .g. done=True only at
...
# CPU observation transport for persistent environment workers.
# Only num_nodes=0 and the (1, 9) float32 feature tensor cross a pipe.
...
# Persistent workers use Windows spawn on every platform.
# The parent alone samples instances; workers own the ALNS RNG state.
...
# Training orchestration and checkpoint publication.
