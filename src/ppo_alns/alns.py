"""Combinatorial search mechanics for PPO-ALNS.

This module deliberately contains no standalone operator selector or search
driver. The PPO actor in :mod:`ppo_alns.ppo` selects one of the nine joint
destroy/repair actions and calls :func:`apply_actor_action` for exactly one
search transition. Geometry and travel coefficients are loaded from the
preprocessed instance and are never reconstructed here.

Solutions map each truck to an ordered stop list. A customer stop serves one
customer directly; a parking stop launches robot trips whose ``ret_p`` names
a later retrieval stop. Every proposed route is checked by ``eval_truck``.

For example, ``action=0`` means random removal followed by greedy repair,
while ``action=8`` means related removal followed by regret-2 repair. The PPO
environment decides whether the returned candidate is accepted.
"""
...
class Solution:
    """Mutable truck routes with explicit robot deployment/retrieval links.

    A direct stop is ``{"kind": "cust", "c": customer_id}``. Parking stops
    store ``p`` and a ``deploys`` list; each trip has ``r``, ``custs``, and
    ``ret_p``. Call ``clone()`` before applying an in-place destroy operator
    so an unsuccessful candidate cannot modify the incumbent solution.
    """
    ...
    def clone(self):
        """Copy routes and all nested stop/trip containers independently."""
    ...
    def used_copies(self):
        """Return parking-copy IDs already assigned to any truck route."""
        out = set()
        for route in self.routes.values():
            for st in route:
                if st["kind"] == "park":
...
def apply_insertion(sol, apply_tuple):
    """Install the selected route without mutating shared candidate stops."""
...
def repair_greedy(pr, sol, pool, rng, noise=0.0):
    """Insert the cheapest feasible customer until the removed pool is empty.

    All remaining customers compete at each round. With ``noise > 0`` each
    feasible insertion receives the configured random cost perturbation.
    Return ``False`` when no remaining customer has a feasible insertion;
    the caller's full-solution check then rejects incomplete coverage.
    """
    remaining = list(pool)
    while remaining:
        best = None
...
def destroy_random(pr, sol, q, rng):
    """Remove up to q distinct customers using the environment's own RNG."""
...
# ============================================================
# 3. Processed-instance provider for PPO training and testing
# ============================================================
...
def _cached_params(cache, path, params_path, size):
    """Use the unchanged validated loader with a process-local cache."""
...
class InstanceCache:
    """Worker-local loader with no sampling RNG or sample() method."""
...
class DirectoryInstanceProvider:
    ...
    def worker_spec(self):
        """Primitive loader settings, never provider RNG state or cached Params."""
        params_path = (
            DEFAULT_PARAMS_PATH
