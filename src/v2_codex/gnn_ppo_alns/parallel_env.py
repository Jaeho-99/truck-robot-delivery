...
def _alns_worker_main(worker_id, connection, cache_init, cfg, norms, codec,
    ...
    try:
        ...
        transport_package = ("v2_codex.gnn_ppo_alns" if package == "v2_codex.gnn_ppo_alns"
                             and cfg.use_graph else "v2_codex.ppo_alns")
        encode_observation = importlib.import_module(
            f"{transport_package}.observation").encode_observation
        builder = None
        if package == "v2_codex.gnn_ppo_alns" and cfg.use_graph:
...
class ParallelVecALNS:
    ...
    def __init__(self, cfg, provider, norms, *, observation_codec="direct",
                 ...
                 package="v2_codex.gnn_ppo_alns"):
        if package not in {"v2_codex.gnn_ppo_alns", "v2_codex.ppo_alns"}:
