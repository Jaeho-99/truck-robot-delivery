...
Run from the repository root::
    ...
    python src/v2_codex/summarize_results.py
    python src/v2_codex/summarize_results.py --size 20

Run this script after each new training or test setting. Legacy root artifacts
and labelled nSIZE/runs/LABEL directories are both discovered. New experiment
commands refuse existing output targets; cumulative tables are still updated
in place by this summarizer.
"""
...
OUTPUT_DIR = REPO_ROOT / "output" / "v2_codex"
MODELS_DIR = REPO_ROOT / "models" / "v2_codex"
...
_COMPARISON_VALUE_FIELDS = {
    ...
    "main_wall_seconds", "solve_wall_seconds", "evaluation_wall_seconds",
}
    ...
    for method_dir in sorted(path for path in OUTPUT_DIR.iterdir()
        ...
        for size_dir in sorted(path for path in method_dir.iterdir()
            ...
            if match is not None:
                ...
                labelled_runs = size_dir / "runs"
                if labelled_runs.is_dir():
                    found.extend((method, tag, int(match.group(1)), run_dir)
                                 for run_dir in sorted(labelled_runs.iterdir())
                                 if run_dir.is_dir())
    return found
    ...
    for method, tag, size, directory in _runs(TRAINED_METHODS):
        for log_path in sorted(directory.glob("train_reward_*.csv")):
            try:
                ...
                if metadata:
                    if metadata.get("schema_version", 1) >= 2 and metadata.get("status") != "completed":
                        raise ValueError("run metadata is not marked completed")
                    _validate_metadata(
                    ...
                    checkpoint = Path(checkpoint_label)
                    ...
                    if directory.parent.name == "runs":
                        raise ValueError("labelled run has no completed training metadata")
                ...
                row: dict[str, Any] = {
                    ...
                    "episodes": len(log),
                    "final_step": metadata.get("effective_steps", max(step_values)),
        ...
        for result_path, reward_token in _result_files(method, directory):
            try:
                ...
                if metadata:
                    ...
                    if directory.parent.name == "runs":
                        raise ValueError("labelled run has no completed test metadata")
                ...
                row: dict[str, Any] = {
                    ...
                    "run_label": metadata.get("run_label", ""),
                    "main_wall_seconds": metadata.get("main_wall_seconds", ""),
                    "solve_wall_seconds": metadata.get("solve_wall_seconds", ""),
                    "evaluation_wall_seconds": metadata.get("evaluation_wall_seconds", ""),
                }
                row.update({key: value for key, value in settings.items()
                            if key not in row})
                by_size.setdefault(size, []).append(row)
