...
def _close_case_workers(processes, connections, idle, grace_seconds=5.0):
    ...
    for process in processes.values():
        if process.is_alive():
            print(f"[alns cleanup] worker PID {process.pid} did not exit; "
...
def run_cases(instance_paths, *, size, params_path=DEFAULT_PARAMS_PATH,
    ...
    if workers == 1:
        ...
        for task in tasks:
            if progress:
                print(f"[alns {task[0] + 1}/{len(tasks)}] "
    ...
    try:
        ...
        while len(ready_workers) != worker_count:
            ...
            if progress and now - last_heartbeat >= heartbeat_interval:
                print(f"[alns startup] waiting={sorted(set(processes) - ready_workers)} "
        ...
        while completed < len(tasks):
            ...
            for worker_id in list(assignments):
                ...
                completed += 1
                if progress:
                    print(f"[alns completed={completed}/{len(tasks)}] "
...
def main():
    ...
    with reserve_artifacts([*route_paths, summary_path, metadata_path]):
        ...
        print(f"[alns] cases={len(cases)} seeds={args.seeds} "
        ...
        print(f"[alns complete] tasks={len(results)} "
