...
# ``python src/ppo_alns/test.py ...``.  In that mode Python otherwise adds
# only ``src/ppo_alns`` (not ``src``) to sys.path.
if __package__ in (None, ""):
    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from common.params import DEFAULT_PARAMS_PATH
from v2_claude.common.policy_evaluation import run_evaluation_cli
from v2_claude.ppo_alns.ppo import REWARD_MODES
...
def main():
    ...
    run_evaluation_cli("v2_claude.ppo_alns", _parser().parse_args(),
