...
Copy of ``src/common/policy_evaluation.py``. The only differences are
that ``package`` is now a dotted name ("v2_claude.ppo_alns"), so the
family that names checkpoints and output directories is its last
segment, and that artifacts live under ``models/v2_claude/`` and
``output/v2_claude/``. No evaluation logic is changed.
...
def _worker(worker_id, connection, package, model_path, metadata, cache_spec):
    ...
    try:
        ...
        builder = (importlib.import_module(f"{package}.gnn").GraphBuilder(norms, cfg)
                   if package.rsplit(".", 1)[-1] == "gnn_ppo_alns"
                   else None)
...
def evaluate_cases(package, model, cfg, builder, provider, model_path, jobs, *,
    ...
    """
    if package not in {"v2_claude.ppo_alns", "v2_claude.gnn_ppo_alns"}:
...
def run_evaluation_cli(package, args, *, main_started=None):
    ...
    model_path = (args.checkpoint or REPO_ROOT / "models" / "v2_claude"
                  / f"{family}_n{args.size}_{reward_token}{suffix}.pt")
    # A run label labels outputs only: choosing a trained model stays explicit.
    method_dir = family if args.tag is None else f"{family}_{args.tag}"
    ...
    with reserve_artifacts([*record_paths, csv_path, metadata_path]):
        ...
        metadata = {
            "schema_version": 2, "status": "completed", "model": family,
            "variant": "v2_claude",
