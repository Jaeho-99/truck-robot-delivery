"""Evaluate PPO and GNN-PPO checkpoints on precomputed test instances.

Each instance/seed pair has its own action generator. Serial evaluation and
Cross-platform spawn workers execute the same evaluator and return results in input
order. Checkpoint loading, worker cleanup, and result publication are shared
by both evaluation entry points.
