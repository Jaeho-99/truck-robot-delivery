"""Strict shared parameters and immutable processed-instance loading."""
...
def _require_real(value: Any, name: str, *, positive: bool = False,
...
class GeometryParams:
    ...
    def __post_init__(self) -> None:
        ...
        _require_real(self.scaling_factor, "geometry.scaling_factor",
                      positive=True)
...
class FleetParams:
    ...
    def __post_init__(self) -> None:
        ...
        _require_int(self.n_robots_per_truck,
                     "fleet.n_robots_per_truck", positive=True)
    ...
    def n_trucks_for(self, size: int) -> int:
        ...
        try:
            ...
            raise ValueError(
                f"unsupported size {size}; choose 5, 10, 20, 50, or 100") from exc
...
class TruckParams:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("fixed_cost", "fuel_cost_per_min", "env_cost_per_min",
                     "service_time_min"):
            _require_real(getattr(self, name), f"truck.{name}",
                          nonnegative=True)
...
class RobotParams:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ("fixed_cost", "fuel_cost_per_min", "env_cost_per_min",
                     "service_time_min", "unload_time_min", "load_time_min"):
            _require_real(getattr(self, name), f"robot.{name}",
                          nonnegative=True)
...
class Params:
    ...
    def __post_init__(self) -> None:
        ...
        _require_real(self.lateness_cost_per_min,
                      "lateness.cost_per_min", nonnegative=True)
        demand = _require_int(self.demand_per_customer,
                              "demand_per_customer", positive=True)
        if demand != 1:
            raise ValueError("demand_per_customer is fixed at 1")
        _require_real(self.time_window_hours, "time_window.hours",
                      positive=True)
...
def _float_field(mapping: Mapping[Any, Any], key: str, path: str,
                 *, positive: bool = False,
                 nonnegative: bool = False) -> float:
    return _require_real(mapping[key], f"{path}.{key}", positive=positive,
                         nonnegative=nonnegative)


def _int_field(mapping: Mapping[Any, Any], key: str, path: str,
               *, positive: bool = False) -> int:
    return _require_int(mapping[key], f"{path}.{key}", positive=positive)


def _parse_params(raw: Any) -> Params:
    root = _exact_keys(raw, {
        "version", "geometry", "fleet", "truck", "robot", "lateness",
        "demand_per_customer", "time_window",
    }, "root")
    ...
    geometry_raw = _exact_keys(root["geometry"], {
        "metric", "scaling_factor", "path_convention", "n_samples",
    }, "geometry")
    ...
    fleet_raw = _exact_keys(root["fleet"], {
        "n_trucks", "n_robots_per_truck",
    }, "fleet")
    ...
    truck_raw = _exact_keys(root["truck"], {
        "speed_kmh", "capacity", "range_km", "fixed_cost",
        "fuel_cost_per_min", "env_cost_per_min", "service_time_min",
    }, "truck")
    truck = TruckParams(
    ...
    robot_raw = _exact_keys(root["robot"], {
        "speed_kmh", "capacity", "range_km", "fixed_cost",
        "fuel_cost_per_min", "env_cost_per_min", "service_time_min",
        "unload_time_min", "load_time_min",
    }, "robot")
    robot = RobotParams(
    ...
    lateness_raw = _exact_keys(
        root["lateness"], {"cost_per_min"}, "lateness")
...
def load_params(path: str | Path | None = None) -> Params:
    ...
    resolved = (DEFAULT_PARAMS_PATH if path is None else Path(path)).expanduser()
...
def zone_source_info() -> tuple[str, str]:
    ...
    method = POLYGON_ZONE_METHOD if DONG_POLYGON_PATH.is_file() else CELL_ZONE_METHOD
    ...
    for path in sources:
        ...
        with path.open("rb") as handle:
            for chunk in iter(lambda: handle.read(1024 * 1024), b""):
                digest.update(chunk)
        source_digests[path.relative_to(REPO_ROOT).as_posix()] = digest.hexdigest()
    canonical = json.dumps(
        {"method": method, "sources": source_digests}, sort_keys=True,
...
def _stored_signature(meta: Mapping[str, Any]) -> dict[str, Any]:
    ...
    geometry = _exact_keys(meta["geometry"], {
        "metric", "scaling_factor", "path_convention", "n_samples",
    }, "meta.geometry")
...
def _check_preproc_compatibility(meta: Mapping[str, Any], params: Params,
                                 npz_path: Path) -> None:
    required = {
        "instance_id", "n_customers", "n_zones", "num_parking_copies",
        "geometry", "truck", "robot", "version", "preproc_hash",
        "preprocess_git_commit", "timestamp", "zone_assignment_method",
        "zone_source_hash", "node_zone_assignment_mismatch_count",
    }
    missing = required - set(meta)
    if missing:
        raise ValueError(
    ...
    if (not isinstance(meta["preprocess_git_commit"], str)
            or not meta["preprocess_git_commit"]):
        raise TypeError(
            f"{npz_path}: meta.preprocess_git_commit must be non-empty")
    if (not isinstance(meta["zone_assignment_method"], str)
    ...
    if (not isinstance(zone_source_hash, str) or len(zone_source_hash) != 64
    ...
    mismatch_count = _require_int(
        ...
        "meta.node_zone_assignment_mismatch_count")
    if mismatch_count < 0:
        raise ValueError("meta.node_zone_assignment_mismatch_count must be >= 0")
    ...
    try:
        ...
        raise ValueError(
            f"{npz_path}: meta.timestamp is not valid ISO-8601") from exc
    current_method, current_zone_hash = zone_source_info()
    if (meta["zone_assignment_method"] != current_method
    ...
    if (not isinstance(stored_hash, str) or len(stored_hash) != 64
        ...
        raise ValueError(f"{npz_path}: metadata preproc_hash is missing/invalid")
    ...
    if stored_hash == params.preproc_hash:
        if stored_signature != current_signature:
            raise ValueError(f"{npz_path}: preprocessing hash collision detected")
        return
...
def _validate_instance_arrays(arrays: Mapping[str, np.ndarray],
                              meta: Mapping[str, Any], params: Params,
                              path: Path) -> None:
    ...
    if node_type.shape != label.shape or node_type.dtype.kind != "U":
        raise TypeError("node_type must be a Unicode vector matching node_label")
    n_nodes = label.size
    ...
    for name, index in (("customer_idx", customer),
                        ("parking_idx", parking), ("depot_idx", depot)):
    ...
    if (partition.size != n_nodes
            or not np.array_equal(np.sort(partition), np.arange(n_nodes))):
        raise ValueError("customer/parking/depot indices must partition nodes")
    expected_types = {
        "customer": customer, "parking": parking, "depot": depot,
    }
    ...
    if isinstance(n_zones, bool) or not isinstance(n_zones, int) or n_zones <= 0:
    ...
    def validate_tau_bounds(name: str, tau: np.ndarray, speed: float,
    ...
    validate_tau_bounds(
        "tau_truck", arrays["tau_truck"], params.truck.speed_kmh,
        arrays["alpha_traffic"])
    validate_tau_bounds(
        "tau_robot", arrays["tau_robot"], params.robot.speed_kmh,
        arrays["alpha_ped"])
    ...
    if (isinstance(copies, bool) or not isinstance(copies, int) or copies <= 0
        ...
        raise ValueError("parking count disagrees with meta.num_parking_copies")
    if meta.get("instance_id") != path.stem:
        raise ValueError("meta.instance_id must match the npz filename")


def load_instance(npz_path: str | Path,
                  params: Params | None = None) -> Instance:
    ...
    expected = {
        "node_label", "node_type", "node_xy", "node_zone", "customer_idx",
        "parking_idx", "depot_idx", "d", "tau_truck", "tau_robot",
        "alpha_traffic", "alpha_ped", "meta",
    }
...
def load_problem(npz_path: str | Path,
                 params_path: str | Path | None = None) -> tuple[Params, Instance]:
