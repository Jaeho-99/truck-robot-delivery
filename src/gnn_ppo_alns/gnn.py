"""Build normalized solution graphs and encode them for GNN-PPO.

``compute_norms`` calibrates feature scales from training instances.
``GraphBuilder.build(pr, sol)`` converts a fully repaired solution into a
heterogeneous graph; ``SolutionEncoder`` pools its node representations.
The nine global search features are attached by the PPO environment.

For example, ``builder = GraphBuilder(norms, cfg)`` followed by
``data = builder.build(pr, sol)`` constructs one observation graph on CPU.
"""
...
def compute_norms(prs, pct=0.95):
    ...
    def percentile(values):
        ...
        return max(values[min(len(values) - 1, int(pct * len(values)))], 1e-9)

    return {
        "c_truck": percentile(truck),
        "c_robot": percentile(robot),
        "d_max": percentile(distance),
    }


def robot_served_customers(sol):
    """Collect customers on robot trips, excluding direct truck deliveries."""
    served = set()
...
class GraphBuilder:
    """Convert route structure and precomputed travel data into graph features.

    Build only after repair has restored all customers. Empty edge stores
    are padded so graphs from different routes can be batched safely.
    """
    ...
    def build(self, pr, sol):
        ...
        for c in cust_ids:
            ...
            cust_x.append(
                [px, py, pr.l_c[c] / T, ttd, 1.0 if c in served else 0.0]
            )
        ...
        for route in sol.routes.values():
            ...
            seq = (
                [0]
                + [
                    st["c"] if st["kind"] == "cust" else st["p"]
                    for st in route
                ]
                + [pr.D]
            )
            for i, j in zip(seq, seq[1:]):
                add(
                    ...
                    j,
                    "truck_arc",
                    [pr.tau_truck(i, j) / c_truck, pr.dist(i, j) / d_max],
                )
            for st in route:
                if st["kind"] != "park":
                    continue
        ...
        for key, (src, dst) in edges.items():
            ...
            data[key].edge_attr = torch.tensor(feats[key], dtype=torch.float32)
...
class SolutionEncoder(nn.Module):
    """Apply heterogeneous attention, then jointly pool all node types.

    The result has ``2 * cfg.hidden_dim`` features per graph: mean and max
    node embeddings. The actor adds global search features separately.
    """
    ...
    def __init__(self, cfg):
        ...
        for _ in range(cfg.n_layers):
            ...
            self.norms.append(
                nn.ModuleDict({t: nn.LayerNorm(d) for t in NODE_DIMS})
            )
        self.act = nn.GELU()
    ...
    def forward(self, data):
        ...
        for conv, ln in zip(self.layers, self.norms):
            ...
            x = {
                t: ln[t](x[t] + self.act(out[t])) if t in out else x[t]
                for t in x
            }
        hs, batches = [], []
        for t in NODE_DIMS:
        ...
        return torch.cat(
            [global_mean_pool(h_all, b_all), global_max_pool(h_all, b_all)],
            dim=1,
        )
