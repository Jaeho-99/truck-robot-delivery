...
class Solution:
    """Store each truck's ordered customer and robot-launch parking stops.

    A direct delivery has ``{"kind": "cust", "c": customer_id}``.
    A parking stop additionally holds robot trips in ``deploys``. Clone
    before applying an operator because routes and nested trips are mutable.
    """
