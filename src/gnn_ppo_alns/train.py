...
This entry point validates arguments, reserves output paths, loads instances,
computes feature normalizers, trains the policy, and publishes run metadata.
All file and runtime helpers are local to keep the command self-contained.

Example from the repository root::

    python src/gnn_ppo_alns/train.py --size 20 --run-label example_01

The dataset comes from ``data/processed*/train``; checkpoints are saved in
``models/`` and CSV/JSON training records in ``output/gnn_ppo_alns*/n20/``.
"""
...
from common.runtime import report_training_runtime
...
def _parser():
    ...
    parser.add_argument(
        "--run-label", help="unique output label, independent of dataset --tag"
    )
...
def main():
    ...
    if cfg.n_envs != 10:
        raise ValueError(
            "this experiment requires unchanged PPOConfig.n_envs == 10"
        )
    if cfg.n_updates < 1:
        raise ValueError(
            f"total_steps must permit at least one full rollout "
    ...
    method_dir = (
        "gnn_ppo_alns" if args.tag is None else f"gnn_ppo_alns_{args.tag}"
    )
    ...
    print(
        f"[start] requested_steps={cfg.total_steps} effective_steps={effective_steps} "
        f"n_updates={cfg.n_updates} device={cfg.device} env_backend={args.env_backend} "
        f"worker_count={worker_count} run_label={args.run_label!r}",
        flush=True,
    )
    # Reserve before CUDA initialization, instance loading, or output publication.
    with reserve_artifacts(
        [model_path, csv_path, updates_path, metadata_path]
    ):
