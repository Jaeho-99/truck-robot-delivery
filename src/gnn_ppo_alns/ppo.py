...
persistent CPU workers use common.parallel_env so both backends use the
same state and reward definitions. Device validation stays in the entry points.
...
from ppo_alns.ppo import FeatureBatch, FeatureObservation
...
class RolloutBuffer:
    ...
    def minibatches(self, n_minibatch, generator=None):
        ...
        for chunk in perm.chunk(n_minibatch):
            ...
            batch = batch_observations([self.obs[i] for i in idx])
        ...
        for upd in range(cfg.n_updates):
            ...
            with torch.no_grad():
                last_v = model.get_value(
                    batch_observations(obs).to(device)
                ).cpu()
...
def evaluate_instance(model, cfg, builder, pr, seed=0, sample=True):
    ...
    for _ in range(cfg.search_iterations):
        with torch.no_grad():
            batch = batch_observations([obs]).to(device)
...
def batch_observations(observations):
    if observations and isinstance(observations[0], HeteroData):
        return Batch.from_data_list(observations)
    return FeatureBatch.from_data_list(observations)
